...
**Warning: This firmware version is not supported anymore. See the version for the *STM32F411CEU6* MCU instead.**

### Description

This software is a firmware for the *STM32F103C8T* MCU that turns it into an interface adapter for *BME280* sensor
...
The firmware targets the *STMicroelectronics STM32F103C8T* MCU and relies on the *GNU ARM Embedded* open-source
...
prototyping board with this MCU should be used. The most cost-efficient solution is usage of a *STM32F103C8T Blue Pill*
board that has all the necessary hardware.

The firmware can be uploaded to the *STM32F103C8T* MCU using the *ST-LINK* hardware programmer (must be connected to
the *JTAG/SWD* interface header of the board). The latest version of the *STM32CubeProgrammer* software can be used to
initiate firmware uploading. Also, the *OpenOCD* software is able to upload the firmware, and it can be performed
directly from the *CLion* IDE.
...
To connect a BME280 sensor to the MCU board refer the provided schematic below (assuming the *Blue Pill* MCU and
...
* `Id` - returns an identification string of the *BMEReader* device. On success writes a firmware name, a firmware
  version, and a hex-encoded serial number of the MCU, e.g. `OK; BMEReader; Version: 0.1; SN: 0123456789ABCDEF01234567`.
...
* `Reset` - performs a software reset (reboot) of the MCU. On success no confirmation message is returned, and the
  serial connection is lost. After the device reboots (not longer than 1 second), it will be ready for communication.
...
* **[STM32F103C8 MCU series page](https://www.st.com/en/microcontrollers-microprocessors/stm32f103c8.html)**
